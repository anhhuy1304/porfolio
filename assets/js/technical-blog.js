  ...
  const render_data = (data, currentPage) =>{
    let content = `<div class="row portfolio-container" data-aos="fade-up" data-aos-duration="500">`
    for (var index in data) {
      if(index < currentPage * itemPerPage & index >= (currentPage - 1) *itemPerPage){
        const item = data[index];
        const template = `<div class="col-lg-4 col-md-6 portfolio-item ${item['filter']}">
              <a href='${item['link']}'>
                  <div class="portfolio-wrap" style="height:100%">
    ...
    content = content + '</div>'
    var contentElement = document.getElementById("technical_blog_data");
    contentElement.innerHTML = content;
    AOS.refresh()
  }

  //get data
  fetch('/technical_blog/data.json')
  ...
  // handle change page
  const pageItems = document.querySelectorAll('#technical_blog_nav .pagination .page-item');
  const prevBtn = document.querySelector('#technical_blog_nav  .pagination .page-item-previous');
  const nextBtn = document.querySelector('#technical_blog_nav .pagination .page-item-next');
  let page = document.querySelectorAll('#technical_blog_nav .pagination .page-item.active');
  ...
  const render_nav = (maxPage) =>{
    const Items = document.querySelectorAll('#technical_blog_nav .pagination .page-item');
    Items.forEach(item => {
